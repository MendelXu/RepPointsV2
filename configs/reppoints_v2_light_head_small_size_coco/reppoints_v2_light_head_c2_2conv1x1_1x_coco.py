...
train_cfg = dict(
    init=dict(
        ...
        debug=False),
    heatmap=dict(
        assigner=dict(type='PointHMAssigner',
                      gaussian_bump=True, gaussian_iou=0.7),
        allowed_border=-1,
        pos_weight=-1,
        debug=False),
    refine=dict(
...
optimizer_config = dict(grad_clip=dict(
    max_norm=35, norm_type=2), _delete_=True)
...
train_pipeline = [
    ...
    dict(type='Collect', keys=['img', 'gt_bboxes',
                               'gt_labels', 'gt_sem_map', 'gt_sem_weights']),
]
test_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(
